...
select avg(price) as avgCarsKia from cars where model = 'kia';
# знайти середнє арифметичне цін для кожної марки машин окремо
select avg(price) as avgCars, model from cars group by model ;
...
select * from cars where price >(select avg(price) as Price from cars);
